...
class MongoQueryLocationPois(database: Database) {
  def execute(locationName: String, parameters: LocationPoiParameters, layers: Seq[String]): Seq[LocationPoiInfo] = {
    log.debugElapsed {
      val pipeline = buildPipeline(locationName, parameters, layers)
  ...
  private def buildPipeline(locationName: String, parameters: LocationPoiParameters, layers: Seq[String]): MongoPipeline = {
    Seq(
      filter(
        and(
          Seq(

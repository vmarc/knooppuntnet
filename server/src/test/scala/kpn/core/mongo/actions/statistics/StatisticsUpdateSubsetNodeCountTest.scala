...
class StatisticsUpdateSubsetNodeCountTest extends UnitTest with SharedTestObjects {
  ...
  test("execute") {
    withDatabase { database =>
      ...
      counts should contain(StatisticValue(nl, hiking, "NodeCount", 2))
      counts should contain(StatisticValue(nl, cycling, "NodeCount", 1))
      counts should contain(StatisticValue(de, hiking, "NodeCount", 2))
      counts should contain(StatisticValue(de, cycling, "NodeCount", 1))
    }
  }

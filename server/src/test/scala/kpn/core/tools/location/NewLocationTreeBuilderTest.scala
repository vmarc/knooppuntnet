...
class NewLocationTreeBuilderTest extends UnitTest {
  ...
  private def printTree(locations: Seq[LocationDoc], tree: LocationTree, level: Int = 0): Unit = {
    locations.find(_._id == tree.name).foreach { location =>
      ...
      println(s"$spaces - `${location._id}` ${location.name} $childCount")
  ...
  private def location(_id: String, paths: Seq[LocationPath]): LocationDoc = {
    LocationDoc(
      _id,
      paths,
      Seq.empty,

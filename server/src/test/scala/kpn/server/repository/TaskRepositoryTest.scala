...
class TaskRepositoryTest extends UnitTest {
  ...
  test("OLD: add, list and delete tasks") {
    withCouchRepository { repository =>
  ...
  private def withCouchRepository(f: TaskRepository => Unit): Unit = {
    withCouchDatabase { database =>
      f(new TaskRepositoryImpl(null, database, false))
    }
  }

  private def withRepository(f: TaskRepository => Unit): Unit = {
    withDatabase { database =>
      f(new TaskRepositoryImpl(database, null, true))
    }
  }
}

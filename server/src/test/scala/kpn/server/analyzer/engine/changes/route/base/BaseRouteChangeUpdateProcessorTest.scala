...
class BaseRouteChangeUpdateProcessorTest extends UnitTest with MockFactory {
  ...
  test("update route") {
    ...
    setup.routeRepository.findBaseRouteById.when(*).returns(Some(beforeBaseRouteDoc)).once()
    ...
    setup.baseRouteDocBuilder.build.when(*).returns(afterBaseRouteDoc).once()

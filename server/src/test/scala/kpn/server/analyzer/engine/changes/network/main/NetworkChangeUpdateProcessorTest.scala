...
class NetworkChangeUpdateProcessorTest extends UnitTest with SharedTestObjects {
  ...
  test("removed network node") {
    ...
    val networkInfoChange = process(before, after)
  ...
  private def process(before: NetworkDoc, after: NetworkDoc): NetworkChange = {

...
class NodeFragmentConnectionAnalyzerTest extends UnitTest {
  ...
  test("a node cannot connect to a fragment if it is not the start or endnode of the fragment") {
    assert(!canConnect(RouteType.values.toSeq, SegmentDirection.Both, node4, None))
  }

  test("when no direction requested, then a node can connect to a fragment") {
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Both, node1, None))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Both, node3, None))
  }

  test("a node can connect a fragment without forward or backward role") {
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node1, None))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node3, None))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node1, None))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node3, None))
  }

  test("a node can connect a fragment if the fragment role matches the requested direction") {
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node1, Some("forward")))
    assert(!canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node3, Some("forward")))
    assert(!canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node1, Some("backward")))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Forward, node3, Some("backward")))

    assert(!canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node1, Some("backward")))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node3, Some("backward")))
    assert(canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node1, Some("forward")))
    assert(!canConnect(RouteType.values.toSeq, SegmentDirection.Backward, node3, Some("forward")))
  }

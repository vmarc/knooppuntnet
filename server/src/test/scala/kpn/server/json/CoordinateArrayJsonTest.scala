...
class CoordinateArrayJsonTest extends UnitTest {
  ...
  test("deserializer") {
    ...
    coordinates(0).x should equal(1.1)
    coordinates(0).y should equal(2.2)
    coordinates(1).x should equal(3.3)
    coordinates(1).y should equal(4.4)
    coordinates(2).x should equal(5.5)
    coordinates(2).y should equal(6.6)
  }
}

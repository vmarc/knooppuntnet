...
class MonitorUpdaterTest21_osm_update_relation_id extends MonitorUpdateTest {
  ...
  private var route2: MonitorTestRoute = uninitialized
  ...
  private def verifyUpdatedRoute(group: MonitorGroup, route: MonitorRoute): Unit = {
    ...
    assertEqual(
      ...
      MonitorRoute(
        ...
        relationIds = Seq(route2.relationId),
        bounds = Some(route2.bounds),
        happy = true
      )
    )
  }
  ...
  private def verifyReference2(route: MonitorRoute): Unit = {
    val reference = configuration.monitorRouteRepository.reference(route._id, Some(route2.relationId)).get
    assertEqual(
      reference,
      MonitorReference(
        _id = reference._id,
  ...
  private def verifyState2(route: MonitorRoute): Unit = {
    val state = configuration.monitorRouteRepository.state(route._id, route2.relationId).get
    assertEqual(
      state,
      MonitorState(
        state._id,
        ...
        segments = Seq(MonitorSegment(route2.relationId, 1, route2.lines.head))
      )
    )
  ...
  private def setupLoadStructure(): Unit = {
    ...
    configuration.monitorRouteStructureLoader.load.when(Some(ReferenceTimestamp1), route2.relationId).returns(Some(monitorRouteRelation))
  }

  private def setupLoadTopLevel(): Unit = {
    val relation = route2.overpassTopLevel
    configuration.monitorRouteRelationRepository.loadTopLevel.when(Some(ReferenceTimestamp1), route2.relationId).returns(Some(relation))
  }

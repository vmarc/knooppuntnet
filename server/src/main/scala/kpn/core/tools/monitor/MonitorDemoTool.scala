...
class MonitorDemoTool(database: Database, overpassQueryExecutor: OverpassQueryExecutor) {
  ...
  private def setupGroup(groupName: String, groupDescription: String, routes: Seq[MonitorDemoRoute]): Unit = {
    ...
    routes.foreach { demoRoute =>
      Log.context(demoRoute.name) {
        ...
        val referenceFilename = s"/kpn/monitor/demo/$groupName/${demoRoute.filename}.gpx"
        val route = MonitorRoute(
          _id = ObjectId(),
          groupId = group._id,
          name = demoRoute.name,
          description = demoRoute.description,
          comment = Some("generated by demo tool"),
          relationId = demoRoute.relationId,
          referenceType = Some("gpx"),
          referenceDay = Some(Time.now.toDay),
          referenceFilename = Some(referenceFilename),
          referenceDistance = 0,
          deviationDistance = 0,
          deviationCount = 0,
          osmWayCount = 0,
        ...
        val routeReference = buildRouteReference(route, referenceFilename)
  ...
  private def buildRouteReference(route: MonitorRoute, referenceFilename: String): MonitorRouteReference = {
    val geometry = new MonitorRouteGpxReader().readFile(referenceFilename)
    ...
    MonitorRouteReference(
      ...
      filename = Some(referenceFilename),
      geometry = geoJson
    )
  }

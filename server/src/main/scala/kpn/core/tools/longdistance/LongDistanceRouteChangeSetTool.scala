...
object LongDistanceRouteChangeSetTool {
  def main(args: Array[String]): Unit = {
    val executor = new OverpassQueryExecutorImpl()
    new LongDistanceRouteChangeSetTool(executor).analyze()
  }
}

class LongDistanceRouteChangeSetTool(overpassQueryExecutor: OverpassQueryExecutor) {

  private val routeId = 3121667L
  private val log = Log(classOf[LongDistanceRouteChangeSetTool])

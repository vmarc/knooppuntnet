...
class MongoQueryNodeChanges(database: MongoDatabase) {
  ...
  def execute(parameters: ChangesParameters): Seq[NodeChange] = {

    val timeRange = TimeRange.fromParameters(parameters)

    val filterElements = Seq(
      Seq(equal("nodeChange.key.elementId", parameters.nodeId.get)),
      if (parameters.impact) {
        Seq(equal("nodeChange.impact", true))
      }
      else {
        Seq.empty
      },
      timeRange match {
        ...
        case Some(range) =>
          Seq(
            gt("nodeChange.key.time", range.start),
            lt("nodeChange.key.time", range.end),
          )
      }
    ).flatten
    ...
    log.debugElapsed {
      val collection = database.getCollection("change-nodes")
      val future = collection.aggregate[NodeChangeDoc](pipeline).toFuture()
      val docs = Await.result(future, Duration(60, TimeUnit.SECONDS))
      (s"${docs.size} node changes", docs.map(_.nodeChange))
    }
  }
}

...
class NetworkIdsLoaderImpl(
  ...
  override def loadByType(timestamp: Timestamp, scopedNetworkType: ScopedNetworkType): Seq[Long] = {
    val xmlString = cachingOverpassQueryExecutor.executeQuery(Some(timestamp), QueryNetworkIdsByType(scopedNetworkType))
    val xml = try {

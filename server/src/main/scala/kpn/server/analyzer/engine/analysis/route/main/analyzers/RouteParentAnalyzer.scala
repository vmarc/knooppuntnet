...
@Component
class RouteParentAnalyzer(baseRouteRepository: BaseRouteRepository) extends RouteAnalyzer {
  ...
  private def findParentRoutes(routeId: Long, level: Int, visitedRouteIds: Seq[Long]): Seq[ParentRoute] = {
    if (visitedRouteIds.contains(routeId)) {
      ...
      val routes = baseRouteRepository.parentRoutes(routeId).map(data => ParentRoute(level, data.routeId, data.name))

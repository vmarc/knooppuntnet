 ...
 */
class NodeFragmentConnectionAnalyzer(networkTypes: Seq[NetworkType], direction: SegmentDirection.Value, node: Node, fragment: Fragment) {
  ...
  private def isBicycle: Boolean = networkTypes.contains(NetworkType.cycling)

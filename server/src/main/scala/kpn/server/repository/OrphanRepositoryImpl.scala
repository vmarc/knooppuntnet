...
@Component
class OrphanRepositoryImpl(oldAnalysisDatabase: OldDatabase) extends OrphanRepository {
  ...
  override def orphanRoutes(subset: Subset, timeout: Timeout = Couch.defaultTimeout): Seq[RouteSummary] = {
    ...
    oldAnalysisDatabase.query(AnalyzerDesign, OrphanRouteView, timeout)(false, true, true, country, networkType).map(OrphanRouteView.convert)
  }

  override def orphanNodes(subset: Subset, timeout: Timeout = Couch.defaultTimeout): Seq[NodeInfo] = {
    val country = subset.country.domain
    val networkType = subset.networkType.name
    oldAnalysisDatabase.query(AnalyzerDesign, OrphanNodeView, timeout)(false, true, true, country, networkType).map(OrphanNodeView.convert)
  }
}

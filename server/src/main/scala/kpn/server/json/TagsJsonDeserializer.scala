...
class TagsJsonDeserializer extends JsonDeserializer[Tags] {
  ...
  override def deserialize(jsonParser: JsonParser, deserializationContext: DeserializationContext): Tags = {
    ...
    if (treeNode.isObject) {
      ...
      val tagsNode = treeNode.get("tags")
      val tags = (0 until tagsNode.size()).map { index =>
        val tagNode = tagsNode.get(index)
        jsonParser.getCodec.treeToValue(tagNode, classOf[Tag])
      }
      Tags(tags)
    }
    else {

...
class BaseRoutePathCodec(registry: CodecRegistry) extends Codec[BaseRoutePath] {

  private val longCodec = registry.get(classOf[Long])
  private val stringCodec = registry.get(classOf[String])

  override def decode(bsonReader: BsonReader, decoderContext: DecoderContext): BaseRoutePath = {
  ...
  override def encode(bsonWriter: BsonWriter, value: BaseRoutePath, encoderContext: EncoderContext): Unit = {
  ...
  override def getEncoderClass: Class[BaseRoutePath] = {
    classOf[BaseRoutePath]
  }
}

// this file is generated by kpn.tools.code.CodecWriter

package kpn.tools.code.codecs.generated

import kpn.api.common.changes.filter.ServerFilterOption
import kpn.tools.code.codecs.Codecs
import org.bson.BsonReader
import org.bson.BsonType
import org.bson.BsonWriter
import org.bson.codecs.Codec
import org.bson.codecs.DecoderContext
import org.bson.codecs.EncoderContext
import org.bson.codecs.configuration.CodecRegistry

class ServerFilterOptionCodec(registry: CodecRegistry) extends Codec[ServerFilterOption] {

  private val booleanCodec = registry.get(classOf[Boolean])
  private val longCodec = registry.get(classOf[Long])
  private val stringCodec = registry.get(classOf[String])

  override def decode(bsonReader: BsonReader, decoderContext: DecoderContext): ServerFilterOption = {
    bsonReader.readStartDocument()

    var name: String = null
    var count: Long = 0
    var selected: Boolean = false

    while (bsonReader.readBsonType != BsonType.END_OF_DOCUMENT) {
      val fieldName = bsonReader.readName
      if (fieldName == "name") {
        name = stringCodec.decode(bsonReader, decoderContext)
      }
      else if (fieldName == "count") {
        count = longCodec.decode(bsonReader, decoderContext)
      }
      else if (fieldName == "selected") {
        selected = booleanCodec.decode(bsonReader, decoderContext)
      }
      else {
        Codecs.log.warn(s"Unknown field name: $fieldName in ServerFilterOptionCodec.decode()")
        bsonReader.skipValue()
      }
    }

    bsonReader.readEndDocument()

    ServerFilterOption(
      name,
      count,
      selected,
    )
  }

  override def encode(bsonWriter: BsonWriter, value: ServerFilterOption, encoderContext: EncoderContext): Unit = {
    bsonWriter.writeStartDocument()

    bsonWriter.writeName("name")
    stringCodec.encode(bsonWriter, value.name, encoderContext)

    bsonWriter.writeName("count")
    longCodec.encode(bsonWriter, value.count, encoderContext)

    bsonWriter.writeName("selected")
    booleanCodec.encode(bsonWriter, value.selected, encoderContext)

    bsonWriter.writeEndDocument()
  }

  override def getEncoderClass: Class[ServerFilterOption] = {
    classOf[ServerFilterOption]
  }
}

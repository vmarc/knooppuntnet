...
export class PdfTextDocumentModel {
  ...
  readonly xCircleCenter = this.xCumulativeDistance + this.cumulativeDistanceWidth + PdfPage.spacer + this.circleRadius;
  ...
  readonly columnWidth =
    PdfPage.spacer + this.cumulativeDistanceWidth + this.circleRadius + PdfPage.spacer + this.routeInfoWidth + PdfPage.spacer + PdfPage.spacer;
  readonly rowHeight =
  ...
  nodeIndex(pageIndex: number, columnIndex: number, rowIndex: number): number {
    return pageIndex * this.maxNodesPerPage +

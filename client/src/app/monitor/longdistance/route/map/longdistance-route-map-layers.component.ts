...
export class LongdistanceRouteMapLayersComponent {
  ...
  readonly gpxVisible$ = this.store.select(selectLongdistanceRouteMapGpxVisible);
  readonly gpxOkVisible$ = this.store.select(selectLongdistanceRouteMapGpxOkVisible);
  readonly gpxNokVisible$ = this.store.select(selectLongdistanceRouteMapGpxNokVisible);
  readonly osmRelationVisible$ = this.store.select(selectLongdistanceRouteMapOsmRelationVisible);

  readonly gpxDisabled$ = this.store.pipe(select(selectLongdistanceRouteMapGpxEnabled), map(e => e === false));
  readonly gpxOkDisabled$ = this.store.pipe(select(selectLongdistanceRouteMapGpxOkEnabled), map(e => e === false));
  readonly gpxNokDisabled$ = this.store.pipe(select(selectLongdistanceRouteMapGpxNokEnabled), map(e => e === false));
  readonly osmRelationDisabled$ = this.store.pipe(select(selectLongdistanceRouteMapOsmRelationEnabled), map(e => e === false));
  ...
  gpxVisibleChanged(event: MatCheckboxChange): void {
    this.store.dispatch(actionLongdistanceRouteMapReferenceVisible({visible: event.checked}));
  }

  gpxOkVisibleChanged(event: MatCheckboxChange): void {
    this.store.dispatch(actionLongdistanceRouteMapOkVisible({visible: event.checked}));
  }

  gpxNokVisibleChanged(event: MatCheckboxChange): void {
    this.store.dispatch(actionLongdistanceRouteMapNokVisible({visible: event.checked}));
  }

...
import { actionLongdistanceRouteChangeInit } from '../../../store/monitor.actions';
import { selectLongdistanceRouteChangePage } from '../../../store/monitor.selectors';
import { selectLongdistanceRouteName } from '../../../store/monitor.selectors';

@Component({
  selector: 'kpn-longdistance-route-change-page',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <ul class="breadcrumb">
      <li><a routerLink="/" i18n="@@breadcrumb.home">Home</a></li>
      <li>
        <a routerLink="/monitor/longdistance-routes">Longdistance routes</a>
      </li>
      <li>Route</li>
    </ul>
    ...
    <div *ngIf="response$ | async as response" class="kpn-spacer-above">
      ...
      <div *ngIf="response.result">
        <table class="kpn-table">
          <tbody>
              ...
              <td>
                <div class="kpn-line">
                  ...
                  <kpn-osm-link-change-set
                    [changeSetId]="response.result.change.key.changeSetId"
                  ></kpn-osm-link-change-set>
                  <span>
                    <a
                      class="external"
                      ...
                      [href]="
                        ...
                        response.result.change.key.changeSetId
                      "
            ...
            <tr>
              ...
              <td>
                <kpn-timestamp
                  [timestamp]="timestamp(response.result.change.key.timestamp)"
                ></kpn-timestamp>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td>
                ...
                <div
                  *ngIf="response.result.change.investigate"
                  class="kpn-line"
                >
                ...
                <div
                  *ngIf="response.result.change.routeSegmentCount !== 1"
                  class="kpn-line route-analysis"
                >
                  <mat-icon svgIcon="warning"></mat-icon>
                  <span
                    >Not OK:
                    {{ response.result.change.routeSegmentCount }} route
                    segments</span
                  >
                </div>
                <div
        ...
        <p>
          Total ways={{ response.result.change.wayCount }}, ways added={{
            response.result.change.waysAdded
          }}, ways removed={{ response.result.change.waysRemoved }}, ways
          updated={{ response.result.change.waysUpdated }}
        </p>
        ...
        <div *ngIf="response.result.change.resolvedNokSegments.length > 0">
          <h2>
            ...
            <span class="kpn-thin"
              >({{ response.result.change.resolvedNokSegments.length }})</span
            >
          </h2>
          <div
            *ngFor="
              let segment of response.result.change.resolvedNokSegments;
              let i = index
            "
            class="segment"
          >
        ...
        <div *ngIf="response.result.change.newNokSegments.length > 0">
          <h2>
            ...
            <span class="kpn-thin"
              >({{ response.result.change.newNokSegments.length }})</span
            >
          </h2>
          <div
            *ngFor="
              let segment of response.result.change.newNokSegments;
              let i = index
            "
            class="segment"
          >
...
export class LongdistanceRouteChangePageComponent implements OnInit {
  util = Util;

  readonly routeName$ = this.store.select(selectLongdistanceRouteName);
  readonly response$ = this.store.select(selectLongdistanceRouteChangePage);

  constructor(private store: Store<AppState>) {}

  ngOnInit(): void {
    this.store.dispatch(actionLongdistanceRouteChangeInit());
  }

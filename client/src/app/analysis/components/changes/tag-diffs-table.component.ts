  ...
  template: `
    <div *ngIf="!!tagDiffs">
      ...
      <table class="kpn-table" title="Tag differences" i18n-title="@@tag-diffs.table">
        ...
        <tbody>
        <tr *ngFor="let tagDetail of tagDiffs.mainTags" [ngClass]="{'same': tagDetail.action.name == 'Same'}">
          <td>
            <kpn-tag-diff-action [action]="tagDetail.action"></kpn-tag-diff-action>
          </td>
          <td>{{tagDetail.key}}</td>
        ...
        <tr *ngFor="let tagDetail of tagDiffs.extraTags" [ngClass]="{'same': tagDetail.action.name == 'Same'}">
          <td>
            <kpn-tag-diff-action [action]="tagDetail.action"></kpn-tag-diff-action>
          </td>
          <td>{{tagDetail.key}}</td>

            ...
            >
              ...
              <div class="fact-detail">
                <span i18n="@@subset-facts.routes" class="kpn-label"
                  >{networkFactRefs.factRefs.size, plural, one {1 route} other
                  {{{networkFactRefs.factRefs.size}} routes}}</span
                >
              </div>
    ...
    this.subsetFact$ = this.activatedRoute.params.pipe(
      ...
      tap((subsetFact) =>
        this.subsetInfo$.next(
          this.subsetCacheService.getSubsetInfo(subsetFact.subset.key())
        )
      )
    );
        ...
        this.appService
          ...
          .pipe(
            tap((response) => {
              if (response.result) {
                this.hasFacts =
                  response.result && response.result.networks.size > 0;

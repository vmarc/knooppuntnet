...
export class WayUpdate {
  ...
  public static fromJSON(jsonObject: any): WayUpdate {
    ...
    return new WayUpdate(
      ...
      jsonObject.removedNodes
        ? List(
            jsonObject.removedNodes.map((json: any) => RawNode.fromJSON(json))
          )
        : List(),
      jsonObject.addedNodes
        ? List(jsonObject.addedNodes.map((json: any) => RawNode.fromJSON(json)))
        : List(),
      jsonObject.updatedNodes
        ? List(
            jsonObject.updatedNodes.map((json: any) =>
              NodeUpdate.fromJSON(json)
            )
          )
        : List(),

...
export class NodeRouteReferenceDiffs {
  ...
  public static fromJSON(jsonObject: any): NodeRouteReferenceDiffs {
    ...
    return new NodeRouteReferenceDiffs(
      jsonObject.removed
        ? List(jsonObject.removed.map((json: any) => Ref.fromJSON(json)))
        : List(),
      jsonObject.added
        ? List(jsonObject.added.map((json: any) => Ref.fromJSON(json)))
        : List(),
      jsonObject.remaining
        ? List(jsonObject.remaining.map((json: any) => Ref.fromJSON(json)))
        : List()
    );
  }
}

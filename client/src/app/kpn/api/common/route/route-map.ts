...
export class RouteMap {
  ...
  public static fromJSON(jsonObject: any): RouteMap {
    ...
    return new RouteMap(
      ...
      jsonObject.unusedSegments
        ? List(
            jsonObject.unusedSegments.map((json: any) =>
              TrackSegment.fromJSON(json)
            )
          )
        : List(),
      jsonObject.startTentaclePaths
        ? List(
            jsonObject.startTentaclePaths.map((json: any) =>
              TrackPath.fromJSON(json)
            )
          )
        : List(),
      jsonObject.endTentaclePaths
        ? List(
            jsonObject.endTentaclePaths.map((json: any) =>
              TrackPath.fromJSON(json)
            )
          )
        : List(),
      ...
      jsonObject.endNodes
        ? List(
            jsonObject.endNodes.map((json: any) =>
              RouteNetworkNodeInfo.fromJSON(json)
            )
          )
        : List(),
      jsonObject.startTentacleNodes
        ? List(
            jsonObject.startTentacleNodes.map((json: any) =>
              RouteNetworkNodeInfo.fromJSON(json)
            )
          )
        : List(),
      jsonObject.endTentacleNodes
        ? List(
            jsonObject.endTentacleNodes.map((json: any) =>
              RouteNetworkNodeInfo.fromJSON(json)
            )
          )
        : List(),
      jsonObject.redundantNodes
        ? List(
            jsonObject.redundantNodes.map((json: any) =>
              RouteNetworkNodeInfo.fromJSON(json)
            )
          )
        : List(),

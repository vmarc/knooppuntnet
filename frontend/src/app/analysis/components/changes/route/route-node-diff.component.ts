...
@Component({
  ...
  template: `
    ...
    <span *ngIf="action === 'added'" class="kpn-label" i18n="@@route-changes.node-diff.added"
      >added</span
    >
    <span *ngIf="action === 'removed'" class="kpn-label" i18n="@@route-changes.node-diff.removed"
      >removed</span
    >

    <div class="kpn-comma-list">
      <kpn-link-node *ngFor="let node of nodeRefs" [nodeId]="node.id" [nodeName]="node.name" />
    </div>
  `,
  standalone: true,

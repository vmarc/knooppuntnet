  ...
  template: `
    <kpn-indicator-dialog letter="N" i18n-letter="@@network-indicator.letter" [color]="color">
      ...
      <markdown dialog-body *ngIf="isOrange()" i18n="@@network-indicator.orange.text">
        This node is included as a member in the network relation. We did not expect this, because
        all routes to this node have role *"connection"*. This would mean that the node is part of
        another network. We expect that the node is not included in the network relation, unless it
        receives the role *"connection"*.
      </markdown>
      ...
      <markdown dialog-body *ngIf="isGray()" i18n="@@network-indicator.gray.text">
        This node is not included as a member in the network relation. This is OK. This node must
        belong to a different network, because all routes to this node within this network have the
      ...
      <markdown dialog-body *ngIf="isRed()" i18n="@@network-indicator.red.text">
        ...
        belongs to another network (all routes to this node have role *"connection"* in the network
        relation), then the node does not have to be included as member in the network relation. The
        node can be added the network relation, but should get the role *"connection"* in that case.
      </markdown>

...
import kpn.planner.domain.poi.TagInformation;
...
public class PoiDeserializer extends JsonDeserializer<Poi> {
	...
	private void parseFileName(TagInformation information) {
		...
		String fileName = parseableFileName.contains("File%") ?
				parseableFileName.substring(parseableFileName.indexOf("File%3A") + 7) :
				parseableFileName.substring(parseableFileName.indexOf("File:") + 5);

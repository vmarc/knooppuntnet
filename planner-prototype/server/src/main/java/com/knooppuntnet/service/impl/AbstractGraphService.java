...
import com.knooppuntnet.domain.RoutingEdge;
import com.knooppuntnet.domain.Section;
import com.knooppuntnet.domain.document.*;
import com.knooppuntnet.domain.document.Map;
import com.knooppuntnet.repository.CouchDbRepository;
...
public abstract class AbstractGraphService {
	...
	private List<NodeMapper> getNodeMappers(List<Long> nodeList) {
		return IntStream.range(0, nodeList.size() - 1)
				.filter(i -> !nodeList.get(i).equals(nodeList.get(i + 1)))
				.mapToObj(i -> new NodeMapper(nodeList.get(i), nodeList.get(i + 1)))
	...
	private List<Long> getNodesFromDijkstra(List<Long> nodes) {
		...
		IntStream.range(0, nodes.size() - 1)
				.mapToObj(i -> this.dijkstraShortestPath.getPath(nodes.get(i), nodes.get(i + 1)).getVertexList())
	...
	private List<Segment> getSegmentsForTentaclePaths(Section section, List<Path> tentaclePaths) {
		Optional<Path> path = tentaclePaths
				.stream()
				.filter(p -> p.getStartNodeId().equals(section.getStartNodeId()))
				.filter(p -> p.getEndNodeId().equals(section.getEndNodeId()))
				.findAny();

		if (!path.isPresent()) {
			path = tentaclePaths
					.stream()
					.filter(p -> p.getStartNodeId().equals(section.getEndNodeId()))
					.filter(p -> p.getEndNodeId().equals(section.getStartNodeId()))
					.findAny();
		}

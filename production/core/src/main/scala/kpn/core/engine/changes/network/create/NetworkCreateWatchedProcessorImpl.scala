...
class NetworkCreateWatchedProcessorImpl(
  ...
  private val log = Log(classOf[NetworkCreateWatchedProcessorImpl])
  ...
  def process(context: ChangeSetContext, loadedNetworkAfter: LoadedNetwork): ChangeSetChanges = {
    ...
    val networkAfter = networkAnalyzer.analyze(networkRelationAnalysis, loadedNetworkAfter.data, loadedNetworkAfter.relation.id)

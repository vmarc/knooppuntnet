...
object NodePageExample {
  ...
  private val nodeInfo: NodeInfo = NodeInfo(
    id = 1,
    active = false,
    ignored = true,
    display = true,
  ...
  private val nodeReferences: NodeReferences = NodeReferences(
  ...
  private val changes: Seq[NodeChangeInfo] = Seq(
    NodeChangeInfo(
      1,
      Some(1),
      ChangeKey(
  ...
  private val totalCount: Int = changes.size

  val nodeDetailsPage: NodeDetailsPage = NodeDetailsPage(nodeInfo, nodeReferences)

  val nodeMapPage: NodeMapPage = NodeMapPage(nodeInfo)

  val nodeChangesPage: NodeChangesPage = NodeChangesPage(nodeInfo, changes, incompleteWarning, totalCount)

  val page: NodePage = NodePage(nodeInfo, nodeReferences, NodeChangeInfos(changes, incompleteWarning = true))
}

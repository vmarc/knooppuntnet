...
class AnalysisRepositoryImpl(
  ...
  override def lastUpdated(): Option[Timestamp] = {
    database.optionGet(lastUpdatedDocumentKey).map(timestampDocFormat.read).map(_.value)
  }

  override def saveLastUpdated(timestamp: Timestamp): Unit = {
    val rev = database.optionGet(lastUpdatedDocumentKey, Couch.batchTimeout).map(timestampDocFormat.read).flatMap(_._rev)
    database.save(lastUpdatedDocumentKey, timestampDocFormat.write(TimestampDoc(lastUpdatedDocumentKey, timestamp, rev)))
  }

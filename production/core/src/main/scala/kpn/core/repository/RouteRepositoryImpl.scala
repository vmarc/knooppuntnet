...
class RouteRepositoryImpl(database: Database) extends RouteRepository {
  ...
  private def readExistingRouteDocs(routes: Seq[RouteInfo]): Seq[RouteDoc] = {
    ...
    database.objectsWithIds(routeDocIds, Couch.batchTimeout, stale = false).map(jsValue => routeDocFormat.read(jsValue))
  }
  ...
  override def delete(routeIds: Seq[Long]): Unit = {
    val routeDocIds = routeIds.map(docId)
    database.deleteDocs(routeDocIds)
  }

  override def routeWithId(routeId: Long, timeout: Timeout = Couch.defaultTimeout): Option[RouteInfo] = {
    database.optionGet(docId(routeId), timeout).map(routeDocFormat.read).map(_.route)
  }

  override def routesWithIds(routeIds: Seq[Long], timeout: Timeout): Seq[RouteInfo] = {
    val ids = routeIds.map(id => docId(id))
    database.objectsWithIds(ids, timeout, stale = false).map(doc => routeDocFormat.read(doc)).map(_.route)
  }
